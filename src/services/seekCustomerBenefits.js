...
async function login(frame, {cpf, user, password }){
    ...
    const alert = await frame.evaluate( () => { return document.querySelector('#alert-1-msg'); })
...
async function logout(frame){
    await frame.evaluate( () => {
        const button = Array.from(document.querySelectorAll('ion-item')).find(x => x.innerText == "Deslogar")
        if(button){
            button.click();
        }
    })
